...
//
// Configuration file for ForwardCom soft core model A
//
//////////////////////////////////////////////////////////////////////////////////
...
// Remember to change the clock frequency if you change the configuration!
// Comment out next line for 32-bit registers, or uncomment for 64-bit registers:
//`define SUPPORT_64BIT

`ifdef SUPPORT_64BIT
    `define RB                    64    // size of general purpose registers, 64 bits
    `define RB1                   63    // index of most significant bit
    `define CLOCK_FREQUENCY    56000000 // max clock frequency for 64-bit version is approx. 56 MHz
`else
    `define RB                    32    // size of general purpose registers, 32 bits
    `define RB1                   31    // index of most significant bit
    `define CLOCK_FREQUENCY    64000000 // max clock frequency for 64-bit version is approx. 68 MHz
`endif 

// IMPORTANT  (Xilinx-specific:)
// The clock generator must match the frequency given above.
...
// Serial input/output BAUD rate. 8 data bits, no parity, 1 stop bit 
// (increasing the BAUD rate further doesn't help. The serial communication speed is limited by other factors)
...
//`define LOADER_FILE "loader_debug.mem"       // filename of hex file of machine code for loader, with debug break

`define MAX_LOADER_SIZE 32'H110        // >= size of loader code, in 32-bit words. Must be even
// (set MAX_LOADER_SIZE to a round number if debugging loader)
